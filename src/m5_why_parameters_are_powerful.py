...
def main():
    ...
    #even_better_draw_circles(rg.Point(150, 75), 20, 7, 'blue', 6)
    #even_better_draw_circles(rg.Point(-150,60), 4, 18, 'green', 2)
    for k in range(8):
        even_better_draw_circles(rg.Point(10 * k, 75), 20 + (2*k), 7, 'blue', 6)
        even_better_draw_circles(rg.Point(-10 * k, 60), 4 + (3*k), 18, 'green', 2-(k-7))
...
def even_better_draw_circles(point, varyby, numberof, color, thickness):
    ...
    for k in range(numberof):
        ...
        turtle.right(90)
        turtle.forward(varyby)
        turtle.left(90)

        turtle.pen_down()
        turtle.draw_circle(varyby * k)
